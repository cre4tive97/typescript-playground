...
let 오늘먹은거: [string, number, boolean] = ["컵누들", 200, true];
...
function tuple(...x: [string, boolean, ...(number | string)[]]) {}

tuple("a", true, 6, 4, "asdf", 1, 2, "sdf");
...
function tuple2(...item: [...(number | string)[]]): [string[], number[]] {
  let strings: string[] = [];
  let numbers: number[] = [];
  item.forEach((a) => {
    if (typeof a == "string") {
...
let tupleResult = tuple2("b", 5, 6, 8, "a");
